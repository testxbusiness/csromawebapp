...
| P1.5 Aggiornare dipendenze vulnerabili | **Parzialmente completato** | Aggiunti override npm per `postcss@8.5.25` e `sharp@0.35.3`; audit production passato da 5 vulnerabilità a 1. Build, TypeScript, lint e smoke test `xlsx` superati. `xlsx@0.18.5` resta intenzionalmente in stand-by per preservare l'import dei calendari e dei risultati dei campionati; serve un task separato per sostituirlo o isolarne l'esecuzione. |
...
| P2.3 Uniformare errori, loading ed empty state | **Completato** | Creati i componenti condivisi `LoadingState`, `EmptyState` ed `ErrorState` in `src/components/ui/FeedbackState.tsx`; applicati alle pagine e sezioni principali di admin, coach e atleta, ai messaggi, calendari, pagamenti, documenti, campionati, dashboard, modal condivisi, protezione autenticazione e report finanziari. Gli stati contestuali nei select e nei filtri restano intenzionalmente specifici del controllo. Build, lint e TypeScript superati. |
