...
| P2.3 Uniformare errori, loading ed empty state | **In corso** | Creati i componenti condivisi `LoadingState`, `EmptyState` ed `ErrorState` in `src/components/ui/FeedbackState.tsx`; applicati a calendario e quote atleta, pagamenti coach, messaggi admin, eventi admin, calendario coach, attività admin, utenti admin, pagamenti admin, documenti admin, palestre admin, squadre admin, stagioni admin, collaboratori admin, messaggi atleta/coach e quote associative admin. Restano dashboard, alcuni pannelli condivisi e le sezioni finanziarie secondarie. |
