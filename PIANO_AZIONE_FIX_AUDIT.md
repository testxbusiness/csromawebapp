...
| P2.2 Ridurre componenti monolitici | **In corso** | Estratti i tipi, mapping e helper comuni dei campionati in `src/components/championship/types.ts`, il catalogo in `useChampionshipCatalog.ts` e il caricamento girone in `useChampionshipGroupDetails.ts`, ora utilizzati dai manager admin, coach e atleta. Nei dashboard atleta/coach il client Supabase è stabile e i caricamenti sono callback memoizzate, evitando riesecuzioni e closure obsolete. I manager conservano mutazioni e rendering, mentre fetching catalogo/dettagli è separato. La refactor procederà per feature e ruoli, con TypeScript, lint, build e test manuali dopo ogni estrazione. |
