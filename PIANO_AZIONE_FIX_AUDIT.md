...
| P2.1 Correggere gli hook React | **Parzialmente completato** | Corretti `AdminDashboard`, `ActivitiesManager`, `GymsManager`, `SeasonsManager`, `BalanceDashboard`, `BalanceReport`, `BulkGenerateModal`, `DocumentsManager`, `EventsManager`, `MembershipFeesManager`, `PaymentsManager`, `TeamAssignmentModal`, `TeamModal`, `TeamsManager`, `AthleteDetailDrawer`, `AthleteFeesManager`, `CoachesManager` e `useAuth`. Dashboard atleta/coach e i tre `ChampionshipsManager` sono rinviati a P2.2, dove verranno scomposti e stabilizzati senza loop nel bootstrap. |
| P2.2 Ridurre componenti monolitici | **In corso** | Estratti i tipi, mapping e helper comuni dei campionati in `src/components/championship/types.ts`, il catalogo in `useChampionshipCatalog.ts` e il caricamento girone in `useChampionshipGroupDetails.ts`, ora utilizzati dai manager admin, coach e atleta. Nei dashboard atleta/coach e nei tre manager campionati il client Supabase è stabile, i caricamenti sono callback memoizzate e gli effetti hanno dipendenze esplicite, evitando riesecuzioni e closure obsolete. I manager conservano mutazioni e rendering, mentre fetching catalogo/dettagli è separato. La refactor procederà per feature e ruoli, con TypeScript, lint, build e test manuali dopo ogni estrazione. |
| P2.3 Uniformare errori, loading ed empty state | **Da iniziare** |  |
