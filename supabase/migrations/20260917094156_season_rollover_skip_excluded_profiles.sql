...
-- Excluded profiles are intentionally not validated for rollover eligibility.
-- This keeps legacy/null profile_type rows from blocking an otherwise valid batch.
...
begin
  ...
  if exists (
    select 1 from jsonb_array_elements(coalesce(p_selections, '[]'::jsonb)) a
  ...
  for item in select value from jsonb_array_elements(coalesce(p_selections, '[]'::jsonb)) loop
    ...
    -- An excluded source profile must only contribute to the excluded count.
    -- Do this before reading/validating profile_type so legacy null values do not abort the batch.
    ...
    values (v_profile_id, p_target_season_id, v_profile_type, v_source_status,
    ...
    on conflict (profile_id, season_id) do update set
      profile_type = excluded.profile_type, status = excluded.status,
    ...
    for team_item in select value from jsonb_array_elements(coalesce(item->'teams', '[]'::jsonb)) loop
      ...
      if not exists (select 1 from public.teams t join public.activities a on a.id = t.activity_id where t.id = v_team_id and a.season_id = p_target_season_id) then
  ...
  return jsonb_build_object(
    'batchKey', p_batch_key, 'included', v_included_count, 'excluded', v_excluded_count,
    'withoutTeam', v_without_team_count, 'teamMembers', v_team_member_count,
    'teamCoaches', v_team_coach_count, 'warnings', v_warning_count, 'replayed', false
  );
end;
$$;
