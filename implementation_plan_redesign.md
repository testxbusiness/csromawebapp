...
| G12.8a Isolamento squadre nei selettori e nelle assegnazioni | [ ] | G12.8 | Task emerso dalla verifica staging del 21/09/2026: i selettori admin e alcuni lookup mostrano ancora squadre di più stagioni insieme. Filtrare tramite `teams.activity_id → activities.season_id`, mantenendo lo storico solo quando una stagione viene selezionata esplicitamente; aggiungere validazione server-side della stagione nelle assegnazioni massive e nei relativi endpoint. Screenshot di riferimento: modal assegnazione atleta con squadre `AMA`/`AMA-2627`, `U14`/`U14-2627` e squadre RLS miste. |
