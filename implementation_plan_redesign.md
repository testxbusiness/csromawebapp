...
| G12.8a Isolamento squadre nei selettori e nelle assegnazioni | [!] | G12.8 | Parziale il 21/09/2026: corretto il modal di assegnazione massiva atleta/coach, che ora riceve solo le squadre della stagione selezionata; i filtri squadra di `/admin/atleti` e `/admin/collaboratori` seguono lo stesso contesto e “Tutte le stagioni” non è più ammesso per una nuova assegnazione. Aggiunto `seasonId` obbligatorio allo schema e validazione server-side in `/api/admin/athletes/bulk` e `/api/admin/coaches/bulk`, verificando `teams.activity_id → activities.season_id`. Remediation: `/admin/collaboratori` seleziona la stagione attiva come filtro predefinito (`0904814`); il contesto coach, `/coach/campionati` e `/coach/profile` mostrano solo le squadre della stagione attiva e nascondono i pannelli campionato vuoti quando non esistono campionati correnti. Remediation sicurezza del 21/09/2026: catalogo campionati, dettagli girone e squadre campionato del coach passano da `GET /api/coach/championships`, con autenticazione, verifica ruolo, stagione attiva e assegnazioni; rimossa la lettura browser che generava `42501 permission denied for table championships`, senza concedere `SELECT` ad `anon`. Test mirati 6/6, typecheck, build con variabili Supabase placeholder e diff check superati; restano soltanto i warning Edge Runtime già presenti. Residuo: auditare i lookup squadra degli altri moduli admin privi di un selettore stagione esplicito, definendo per ciascuno il contesto corretto senza nascondere lo storico. Screenshot di riferimento: modal assegnazione atleta con squadre `AMA`/`AMA-2627`, `U14`/`U14-2627` e squadre RLS miste. |
...
| G12.9 Dry-run, esecuzione 2026/2027 e gate | [ ] | G12.1–G12.8d | Backup/evidenze, esecuzione autorizzata, attivazione atomica, conteggi post-run e verifica assenza di perdita dati. |
