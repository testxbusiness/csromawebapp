...
interface Event {
  ...
  start_time: string
  end_time: string
  // Recurrence not supported in this DB schema; kept for UI compatibility
...
export default function CoachCalendarPage() {
  ...
  const loadEvents = async (teamsArg?: Team[]) => {
    ...
    // Step 1: fetch event-team links for coach teams
    const { data: links, error: etErr } = await supabase
      .from('event_teams')
      .select('event_id, team_id')
      .in('team_id', teamIds)
    if (etErr || !links || links.length === 0) {
      setEvents([])
      return
    }
    ...
    try {
      if (editingEvent?.id) {
        ...
        if (eventData.selected_teams.length > 0) {
          const teamAssociations = eventData.selected_teams.map(teamId => ({
        ...
        if (isRecurring) {
          // Build occurrences from recurrence UI fields
          ...
          // Set parent_event_id to the first id in the series for grouping
          if (ids.length > 0) {
  ...
  const deleteEvent = async (id: string) => {
    ...
    if (evt?.event_type === 'recurring' || evt?.parent_event_id) {
      const choice = window.confirm('Questo evento fa parte di una ricorrenza. Premi OK per eliminare TUTTA la serie, oppure Annulla per scegliere di eliminare solo questa occorrenza.')
      if (choice) {
        deleteSeries = true
      } else {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderColor: 'var(--cs-primary)' }}></div>
      </div>
    )
  }
  ...
  return (
    <>
      <PageHeader title="Gestione Calendario" />
      <div className="cs-card">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setViewMode(viewMode==='list'?'calendar':'list')} className={`cs-btn ${viewMode === 'list' ? 'cs-btn--outline' : 'cs-btn--accent'}`}>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="mt-2 text-sm text-secondary space-y-1">
                        ...
                        <div>
                          👥 Squadre: {event.selected_teams.map(teamId => {
              ...
              <div className="cs-modal cs-modal--md" data-state="open">
                <form onSubmit={(e) => {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <label className="cs-input">Data/Ora Fine *</label>
                      <input
                        type="datetime-local"
                        name="end_time"
                        required
                      ...
                      {teams.map((team) => (
                        <label key={team.id} className="flex items-center">
                          <input
                            ...
                            name="teams"
                            value={team.id}
                            defaultChecked={editingEvent?.selected_teams.includes(team.id)}
                            className="mr-2"
                          />
                          {team.name} ({team.code})
                        </label>
                  ...
                  {/* Recurrence options (visible when recurring selected) */}
...
function CalendarBlock({ events, currentDate, onNavigate, view, onViewChange, onEventClick }:{ events: any[]; currentDate: Date; onNavigate: (a:'prev'|'next'|'today')=>void; view: 'month'|'week'; onViewChange: (v:'month'|'week')=>void; onEventClick: (id:string)=>void }) {
  ...
  return (
    <SimpleCalendar currentDate={currentDate} view={view} events={evs} onNavigate={onNavigate} onViewChange={onViewChange} onEventClick={onEventClick} />
  )
}
