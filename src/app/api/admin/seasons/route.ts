...
export async function POST(request: NextRequest) {
  try {
    ...
    if (requested.is_active) {
      return conflict('La bozza deve essere creata inattiva')
    }
    ...
    const exactTarget = seasons.data.find((season) =>
      season.name === requested.name &&
      season.start_date === requested.start_date &&
      season.end_date === requested.end_date,
    )
    if (exactTarget) {
      if (exactTarget.is_active) {
        return conflict('La stagione richiesta esiste già come attiva')
      }
    ...
    const sameNameOrPeriod = seasons.data.find((season) =>
      season.name === requested.name ||
      (season.start_date === requested.start_date && season.end_date === requested.end_date),
    )
    if (sameNameOrPeriod) {
    ...
    const overlappingSeason = seasons.data.find((season) =>
      isOverlapping(requested.start_date, requested.end_date, season.start_date, season.end_date),
    )
    if (overlappingSeason) {
      return conflict('Il periodo si sovrappone a una stagione esistente')
    }
    ...
    if (insertError) {
      return NextResponse.json({ error: 'Impossibile creare la bozza stagione' }, { status: 500 })
    }
