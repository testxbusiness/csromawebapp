...
export const athleteBulkSchema = z.discriminatedUnion('operation', [
  z.object({ operation: z.literal('assign_to_team'), athleteIds: ids, parameters: z.object({ teamId, seasonId: uuid, jerseyNumber: z.string().regex(/^\d{1,2}$/).optional(), membershipFeeId: uuid.optional() }).strict(), dryRun: z.boolean().optional() }).strict(),
...
export const coachBulkSchema = z.discriminatedUnion('operation', [
  z.object({ operation: z.literal('assign_to_team'), coachIds: ids, parameters: z.object({ teamId: teamId.optional(), teamIds: z.array(teamId).min(1).max(100).optional(), seasonId: uuid, role: coachRole.optional() }).strict(), dryRun: z.boolean().optional() }).strict(),
