...
export default function SeasonRolloverWizard({ seasons, onClose, onCompleted }: { seasons: Season[]; onClose: () => void; onCompleted: () => void }) {
  const source = useMemo(() => seasons.find((season) => season.name === 'Stagione 2025/2026'), [seasons])
  const target = useMemo(() => seasons.find((season) => season.name === 'Stagione 2026/2027'), [seasons])
  ...
  if (!source?.id || !target?.id) return <FeedbackState variant="error" title="Prerequisiti incompleti" description="Servono le stagioni 2025/2026 e 2026/2027. Crea prima la bozza target inattiva." action={<Button variant="outline" onClick={onClose}>Chiudi</Button>} />
  if (submitted) return <FeedbackState variant="success" title="Selezione salvata" description="Le iscrizioni target sono state applicate. La stagione 2025/2026 resta invariata e la 2026/2027 resta inattiva." action={<Button onClick={onClose}>Chiudi</Button>} />

  return <section aria-labelledby="season-rollover-title" className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 id="season-rollover-title" className="text-xl font-semibold">Rollover profili 2026/2027</h2><p className="text-sm text-secondary">Bozza inattiva · nessun dato storico viene spostato o cancellato.</p></div>
    ...
    {!loading && step === 'season' ? <Card variant="primary" className="space-y-4"><h3 className="font-semibold">Stagioni coinvolte</h3><div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs text-secondary">Source storica</p><p className="font-medium">{source.name}</p><p className="text-sm text-secondary">{source.start_date.slice(0, 10)} → {source.end_date.slice(0, 10)}</p></div><div><p className="text-xs text-secondary">Target</p><p className="font-medium">{target.name}</p><p className="text-sm text-secondary">{target.start_date.slice(0, 10)} → {target.end_date.slice(0, 10)} · {target.is_active ? 'attiva' : 'inattiva'}</p></div></div><Alert variant="info">Le relazioni globali di profili, account e familiari non vengono duplicate.</Alert></Card> : null}
    ...
    {!loading && step === 'summary' ? <Card className="space-y-5"><h3 className="font-semibold">Riepilogo e conferma</h3><div className="grid gap-3 sm:grid-cols-3"><div className="rounded border p-3"><span className="block text-sm text-secondary">Inclusi</span><strong className="text-xl">{includedCount}</strong></div><div className="rounded border p-3"><span className="block text-sm text-secondary">Esclusi</span><strong className="text-xl">{excludedCount}</strong><p className="text-xs text-secondary">Resteranno nella 2025/2026 e non saranno iscritti alla 2026/2027.</p></div><div className="rounded border p-3"><span className="block text-sm text-secondary">Senza squadra</span><strong className="text-xl">{withoutTeamCount}</strong></div></div><ul className="space-y-1 text-sm">{allProfiles.filter((profile) => !included[profile.profile.id]).slice(0, 12).map((profile) => <li key={profile.profile.id}>{profile.profile.firstName} {profile.profile.lastName} — escluso</li>)}</ul><Alert variant="info">I genitori collegati derivano dalla scelta dell'atleta: profilo, account, relazione e permessi non vengono duplicati o modificati.</Alert><label className="flex items-start gap-3 rounded border p-3"><input type="checkbox" checked={confirm} onChange={(event) => setConfirm(event.target.checked)} className="mt-1 h-5 w-5" /><span>Confermo il riepilogo. Applica solo le iscrizioni e le membership target; non attivare la stagione e non modificare la 2025/2026.</span></label></Card> : null}
