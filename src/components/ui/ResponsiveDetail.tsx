...
export function ResponsiveDetail({ open, onOpenChange, title, description, children, footer, size = 'md', fullscreenOnMobile = false, centeredOnMobile = false, centeredOnDesktop = false, className, canClose }: ResponsiveDetailProps) {
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => { if (!nextOpen && canClose && !canClose()) return; onOpenChange(nextOpen) }}>
      <DialogContent className={cn('cs-responsive-detail', `cs-responsive-detail--${size}`, fullscreenOnMobile && 'cs-responsive-detail--fullscreen-mobile', centeredOnMobile && 'cs-responsive-detail--centered-mobile', centeredOnDesktop && 'cs-responsive-detail--centered-desktop', className)}>
